...
**Status:** partial by specification: T1 is stopped at the documented spec wall in
`C3-HARDEN-3.6-ESCALATION.md`. Nothing here claims round-8 readiness.
...
- T1: not claimed; stopped and escalated before implementation.
...
- T1 (design, pending owner decision): an exit encountering an unexpired prepared intent parks for at most the remaining prepared TTL (default prepared TTL 14 minutes), then may be rerun. The literal route outcome requested by triage is unreachable; see escalation.
...
- False prepared/finalize availability narrative: full exit safety remains T1-blocked; finalize alone refusing a prepared row is not represented as full-flow closure.
- False interaction-2 narrative: a pre-finalize relist cannot catch a later straggler; T2 orders publication before sweep via the retained lease, while T1 awaits the owner decision recorded in escalation.
