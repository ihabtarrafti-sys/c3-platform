...
**Severity:** 🟠 Latent risk
**Sprint attributed:** S18 Phase 4 (known gap)
**File:** `useExecuteApproval.ts`, `SharePointJourneyService.ts`

`PartialExecutionError` is thrown when an approval is stamped `Approved` → `Executed` but
the downstream SP write (e.g., creating the Journey row) partially fails. There is no retry
or compensating transaction. The approval stays `Executed` but the journey may not exist.

**Resolution:** Add idempotent retry guard to `executeApproval` (check for existing journey
before creating) and a UI "Retry Execution" button for `ExecutionFailed` approvals. Not in
S20 scope.
