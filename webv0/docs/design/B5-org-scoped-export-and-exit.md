...
3. **DONE + HOSTED-PROVEN (2026-07-07)** — exit ceremony tooling + runbook: `exit:tenant` (`packages/persistence/src/exitTenant.ts` + CLI, `npm run exit:tenant`). Dry-run default (read-only tx), dual typed confirmation, E1-incomplete refusal (active member holds membership), data-return-first manifest check, single-transaction erasure with trigger disable/re-enable + in-tx zero-row post-checks, shared users preserved. 5 tests incl. full-DB fingerprint proofs that refusals/dry-runs mutate nothing. Runbook: [runbooks/B5-exit-tenant-ceremony.md](../runbooks/B5-exit-tenant-ceremony.md). **Hosted certification: the E-1 drill executed the full ceremony live (export → E1 → dual-authorized erase, clean post-checks, zero orphans, triggers re-armed) — see [certifications/E1-onboarding-dry-run.md](../certifications/E1-onboarding-dry-run.md).**
