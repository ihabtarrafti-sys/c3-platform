...
|---|---|---|---|
| **S41** | **Agreements — the FULL domain** (renamed from Contracts, owner direction: contracts, NDAs, addendums, MOUs in one lifecycle) | Governed AddAgreement / RenewAgreement / TerminateAgreement + direct-audited non-material edits + `linkedAgreementId` (addendum → parent as a first-class relationship) + derived renewal windows (30/60/90) + financial role-gating. Includes the renewal WRITE the CP never shipped. | The first domain that goes *beyond* the CP. Money + governance in one pipeline is the product's spine — and the model generalizes past contracts. |
