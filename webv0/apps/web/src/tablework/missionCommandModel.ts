...
export type MissionCommandModuleId = 'mission-field' | 'mission-current' | 'mission-obligations';
export type MissionCommandVisibility = 'open' | 'minimized' | 'closed';
export type MissionCommandPreset = 'commander' | 'review' | 'brief';
...
const IDS: readonly MissionCommandModuleId[] = ['mission-field', 'mission-current', 'mission-obligations'];
const VISIBILITIES: readonly MissionCommandVisibility[] = ['open', 'minimized', 'closed'];
const LAYOUTS: readonly MissionCommandLayout[] = ['commander', 'review', 'brief', 'custom'];
...
export function restoreMissionCommand(raw: string | null): MissionCommandState {
  if (!raw) return { ...DEFAULT_MISSION_COMMAND, windows: copyWindows(DEFAULT_MISSION_COMMAND.windows) };
  try {
    const parsed = JSON.parse(raw) as Partial<MissionCommandState>;
    if (
      parsed.version !== 1 ||
      !LAYOUTS.includes(parsed.layout as MissionCommandLayout) ||
      !Array.isArray(parsed.windows) ||
      parsed.windows.length !== IDS.length ||
      !parsed.windows.every(isWindow) ||
      new Set(parsed.windows.map((window) => window.id)).size !== IDS.length
    ) {
      return { ...DEFAULT_MISSION_COMMAND, windows: copyWindows(DEFAULT_MISSION_COMMAND.windows) };
    }
    return {
      version: 1,
      layout: parsed.layout as MissionCommandLayout,
      windows: parsed.windows.map((window) => ({ ...window, rect: clampMissionCommandRect(window.rect) })),
    };
  } catch {
    return { ...DEFAULT_MISSION_COMMAND, windows: copyWindows(DEFAULT_MISSION_COMMAND.windows) };
  }
