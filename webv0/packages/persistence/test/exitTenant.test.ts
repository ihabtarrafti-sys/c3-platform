...
async function fingerprint(): Promise<string> {
  return admin(async (c) => {
    const parts: string[] = [];
    for (const t of ['tenant', 'app_user', 'external_identity', 'tenant_membership', 'role_assignment', 'business_id_counter', 'person', 'credential', 'journey', 'approval', 'approval_event', 'audit_event']) {
...
describe('exit ceremony — executed', () => {
  it('erases exactly the target tenant; shared users and the other tenant are untouched; triggers still enforce', async () => {
    ...
    await admin(async (c) => {
      ...
      for (const t of ['person', 'credential', 'journey', 'approval', 'approval_event', 'audit_event', 'tenant_membership', 'role_assignment', 'business_id_counter']) {
