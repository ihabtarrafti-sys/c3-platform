...
export function mapPerson(row: any): Person {
  return {
    ...
    dateOfJoining: row.dateOfJoining ?? row.date_of_joining ?? null,
    position: row.position ?? null,
    otherNationalities: row.otherNationalities ?? row.other_nationalities ?? [],
    photoStorageKey: row.photoStorageKey ?? row.photo_storage_key ?? null,
    photoContentType: row.photoContentType ?? row.photo_content_type ?? null,
    photoSha256: row.photoSha256 ?? row.photo_sha256 ?? null,
    photoUpdatedAt: iso(row.photoUpdatedAt ?? row.photo_updated_at ?? null),
