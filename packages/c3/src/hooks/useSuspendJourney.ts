/**
 * useSuspendJourney.ts
 *
 * Sprint 19 Phase 2 — Journey Lifecycle Transitions.
 *
 * Mutation hook for marking an Active Journey as Suspended.
 *
...
export interface SuspendJourneyVariables extends JourneyTransitionRequest {
...
export const useSuspendJourney = () => {
  ...
  return useMutation({
    mutationFn: ({ journeyId, actorLoginName, reason }: SuspendJourneyVariables) =>
      journeyService.suspendJourney({ journeyId, actorLoginName, reason }),
